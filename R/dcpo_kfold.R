...
dcpo_kfold <- function(dcpo_input,
  ...
  dcpo_stan <- list( K          = max(as.numeric(dat$kk)),
                     ...
                     N_test     = nrow(dat %>% filter(test)),
                     kk_test    = as.numeric(dat %>% filter(test) %>% pull(kk)),
                     tt_test    = as.numeric(dat %>% filter(test) %>% pull(tt)),
                     qq_test    = as.numeric(dat %>% filter(test) %>% pull(qq)),
                     rr_test    = dat %>% filter(test) %>% pull(rr),
                     n_r_test   = dat %>% filter(test) %>% pull(n_r),
